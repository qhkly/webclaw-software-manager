...
API_RESPONSE=$(curl -fsSL "https://data.services.jetbrains.com/products/releases?code=PCP&latest=true&type=release")
VERSION=$(echo "$API_RESPONSE" | python3 -c "import json,sys; d=json.load(sys.stdin); print(d['PCP'][0]['version'])" 2>/dev/null || echo "")
[ -z "$VERSION" ] && echo "[ERROR] 无法获取最新版本" && exit 1
echo "[INFO] 安装 PyCharm Community v${VERSION} (${ARCH})"

if [ "$ARCH" = "arm64" ]; then
    DOWNLOAD_URL=$(echo "$API_RESPONSE" | python3 -c "import json,sys; d=json.load(sys.stdin); print(d['PCP'][0]['downloads']['linuxARM64']['link'])" 2>/dev/null || echo "")
else
    DOWNLOAD_URL=$(echo "$API_RESPONSE" | python3 -c "import json,sys; d=json.load(sys.stdin); print(d['PCP'][0]['downloads']['linux']['link'])" 2>/dev/null || echo "")
fi
