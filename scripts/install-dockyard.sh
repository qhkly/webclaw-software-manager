...
# 检查是否已安装
if dpkg -s dockyard 2>/dev/null | grep -q "Status: install ok installed"; then
    echo "[INFO] dockyard 已安装，跳过"
    exit 0
fi
...
if [ -z "$LATEST_TAG" ]; then
    ...
    LATEST_TAG=$(curl -fsSL -H "User-Agent: webclaw-software-manager/0.1" \
        "https://api.github.com/repos/land007/dockyard/releases/latest" \
...
TMP_DIR=$(mktemp -d)
curl -fsSL --progress-bar -L "$DOWNLOAD_URL" -o "${TMP_DIR}/dockyard.deb"

echo "[INFO] 安装 deb 包..."
sudo dpkg -i "${TMP_DIR}/dockyard.deb" || sudo apt-get install -fy

rm -rf "$TMP_DIR"

mkdir -p /opt/on-demand-icons
ICON_SRC=$(find /usr/share/icons -name "dockyard*" 2>/dev/null | sort -r | head -n1)
[ -n "$ICON_SRC" ] && cp "$ICON_SRC" /opt/on-demand-icons/dockyard.png || true

echo "[INFO] Dockyard 安装完成"
