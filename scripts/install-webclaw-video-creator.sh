...
DEB_PKG="webclaw-video-creator"
REPO="qhkly/webclaw-video-creator"
...
echo "[INFO] 获取 Webclaw Video Creator 最新版本..."
...
if [ "$download_ok" != "true" ]; then
    echo "[ERROR] Webclaw Video Creator 下载失败（3次重试后）"
    rm -rf "$TMP"; exit 1
fi
...
rm -rf "$TMP"
echo "[INFO] Webclaw Video Creator 安装完成"
