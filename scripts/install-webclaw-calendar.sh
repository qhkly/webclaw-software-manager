...
DEB_PKG="webclaw-calendar"
REPO="qhkly/webclaw-calendar"
...
echo "[INFO] 获取 Webclaw Calendar 最新版本..."
...
if [ "$download_ok" != "true" ]; then
    echo "[ERROR] Webclaw Calendar 下载失败（3次重试后）"
    rm -rf "$TMP"; exit 1
fi
...
rm -rf "$TMP"
echo "[INFO] Webclaw Calendar 安装完成"
